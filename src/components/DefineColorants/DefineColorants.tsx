...
import { DefineColorantsInputs, DefineColorantsInputsProps } from "./components/DefineColorantsInputs";
...
export function DefineColorants() {
  ...
  const getDefineColorantsInputs = (entry: Entry) => {
    ...
    const props: DefineColorantsInputsProps = {
      color: entry.color,
    ...
    return <DefineColorantsInputs {...props} />;
  };

  return (
    <div className="mx-auto mt-3 max-w-[600px]">
      <DefineColorantsInputs isHeader />
      <SortableList
        items={entries}
        onChange={setEntries}
